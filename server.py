#!/usr/bin/env python3
"""ARK Dashboard - Bottle HTTP server + async data collector"""
...
def rpc_batch(items):
    try:
        return requests.post(RPC_URL, json=items, timeout=60).json()
    except: return []
...
def collector():
    global DATA, BONUS_DAILY, STAKE_DAILY, LAST_BLOCK
    ...
    while True:
        ...
        try:
            ...
            if current > LAST_BLOCK:
                for addr, store in [(ADDR_BONUS, BONUS_DAILY), (ADDR_STAKE, STAKE_DAILY)]:
                    logs = get_logs(addr, LAST_BLOCK+1, current)
                    if logs:
...
if __name__ == "__main__":
    ...
    run(host="0.0.0.0", port=port, server="waitress" if __import__("importlib").util.find_spec("waitress") else None)
